...
describe('Testing API: 2FA handler', () => {
  ...
  it('should generate a 2fa secret and QRCode URL', async () => {
    ...
    expect(payload.hasOwnProperty('secret') && payload.secret.length > 0).to.equal(true)
    expect(payload.hasOwnProperty('qrCodeUrl') && payload.secret.length > 0).to.equal(true)
  })
})
